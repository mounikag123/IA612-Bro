...
event http_request(c:connection,method: string, original_URI: string, unescaped_URI: string, version: string) {
    ...
    if ( (sensitive_URIs in URI1 ) || (method == "POST" && sensitive_post_URIs in URI1) ){
		print fmt("Evil URI %s", c$id$resp_h);

    }
	if ( (sql_injection_uri in URI1 ) || (method == "POST" && sql_injection_uri in URI1) ){
		print fmt("SQL Injection %s", c$id$resp_h);
